...
|---|---|---|---|---|---|---|---|
| R-01 | Toolchain bootstrap (ATL-007) stalls M0; docs exist but repo can't build | Medium | Medium | All engineering blocked | atlas-cto | ATL-007 is P0 and **in-progress as of 2026-07-21** (delegated to atlas-cto at MISSION-1 kickoff, DL-007); exit gate is timed; ATL-010 QA verification queued behind delivery | Open |
...
| R-08 | Repo bloat/confusion from legacy prototype + committed archives | Low | High (already true) | Slow clones, contributor confusion | atlas-cto | ADR-0007 freeze + M2 deletion; ATL-009 cleanup report (in-progress) first, then ATL-008 deletion only after human owner reviews the report and approves (DL-007) | Open |
