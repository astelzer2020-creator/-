...
describe("loginSchema", () => {
  it("accepts valid credentials", () => {
    expect(loginSchema.safeParse({ email: "a@b.co.il", password: "12345678" }).success).toBe(true);
  });
  ...
  it("rejects a malformed email with the Hebrew i18n message", () => {
    ...
    if (!result.success) {
      expect(issuesToFieldErrors(result.error).email).toBe(t("auth.errors.emailInvalid"));
    }
  });

  it("rejects a short password with the Hebrew i18n message", () => {
    const result = loginSchema.safeParse({ email: "a@b.co.il", password: "1234" });
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(issuesToFieldErrors(result.error).password).toBe(t("auth.errors.passwordMin"));
    }
  });
});
...
describe("scenarioSchema", () => {
  ...
  it("rejects negative money and out-of-range rates with field-level Hebrew messages", () => {
    ...
    if (!result.success) {
      ...
      expect(errors.buildCostPerSqmAgorot).toBe(t("scenario.errors.buildCostPositive"));
      expect(errors.discountRate).toBe(t("scenario.errors.discountRateRange"));
    }
  });

  it("requires at least one apartment-mix row", () => {
    const result = scenarioSchema.safeParse({ ...validScenario(), apartmentMix: [] });
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(issuesToFieldErrors(result.error).apartmentMix).toBe(t("scenario.errors.mixRequired"));
    }
  });
  ...
  it("cross-field: rejects two mix rows with the same room count", () => {
    ...
    if (!result.success) {
      const errors = issuesToFieldErrors(result.error);
      expect(errors["apartmentMix.2.rooms"]).toBe(t("scenario.errors.duplicateRooms"));
    }
  });

  it("marks NaN (unparseable input) as an invalid number in Hebrew", () => {
    const result = scenarioSchema.safeParse({ ...validScenario(), constructionMonths: Number.NaN });
    expect(result.success).toBe(false);
    if (!result.success) {
